...
三炮十五兵是一个 5x5 棋盘上的双人零和游戏：

- 炮方有 3 枚炮。
- 兵方最多有 15 枚兵。
- 任意合法局面都可以用表库查询炮胜、兵胜或和棋。

本项目提供从规则引擎到本地交互界面的完整工具链，可以查询：

- 当前局面的胜负结果：`CannonWin`、`SoldierWin`、`Draw`。
- 推荐下一步与所有合法着法评价。
- WDL 表库中的胜负和结果。
- MTD 表库中的材料目标与保证步数。
- 本地 Web UI 中的棋盘交互、推荐提示、合法着法分组和路线探索。

这是一个本地运行项目，不是在线平台或联网服务。表库和 HTTP API 都运行在用户自己的机器上。

## 游戏规则

棋盘为 5x5，格号按从左到右、从上到下编号：
...
下文命令使用 `.\build-mtd-release\sanpao15_cli.exe` 作为示例。若你的构建器输出到 `Release` 子目录，请相应替换路径。

## 编译

本项目根目录提供 `CMakeLists.txt`，要求 CMake 3.20+ 和 C++20 编译器。
...
WDL 是 Win/Draw/Loss 表库，保存每个合法局面的胜负和结果：
...
请不要把 `.s15res`、`.s15mtd` 或生成目录提交到 Git，也不要上传到默认 release 附件中。
...
当前仓库尚未包含独立 `LICENSE` 文件。正式开源分发前建议补充许可证文件。第三方依赖说明见 [docs/third-party.md](docs/third-party.md)。
