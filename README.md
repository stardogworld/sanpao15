...
`sanpao15` is a first-stage implementation of the 5x5 "三炮十五�? game. It contains a C++20 rules core, a retrograde-analysis solver, a command-line table generator/analyzer, focused C++ tests, and a minimal TypeScript UI.
...
- Cannons capture only in the form `�?- empty - 兵`, landing on the soldier square and removing that soldier.
...
prototype layered on top of WDL. Version 2 is outcome-aware: CannonWin and
SoldierWin report forced win guarantee distance, while Draw reports
`materialTarget`, `cannonMaxCaptures`, `soldierSaved`, and adversarial delay to
that material target. Version 1 prototype files are rejected. The current v2
prototype solves and fully verifies `k=0..4`; see
...
drawing first move is `22->12 �?12`. The panel can reset to the start,
